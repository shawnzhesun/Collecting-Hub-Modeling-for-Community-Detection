 ...
 * George
 *
 * $Id: io.c,v 1.3 2011-04-20 20:02:19 venu Exp $
 *
 */
 ...
 ***********************************************************************/
int
readClustering (const char *filename, int *part, int nvtex)
{
  FILE *fpin;
  int i, numClus = 0;
...
void
ReadMatrix (Matrix * m, char *filename, wgttype threshold)
{
  idxtype *xadj, *adjncy;
  wgttype *adjwgt;
  ...
  m->adjwgt =
    (wgttype *) GKmalloc (sizeof (wgttype) * m->nnz, "ReadMatrix:adjwgt");
...
void
ReadGraph (GraphType * graph, const char *filename, int *wgtflag,
	   int addSelfLoop, int txtFormat)
{
  ...
  fmt = ncon = 0;
  sscanf (line, "%d %d %d %d", &(graph->nvtxs), &(graph->nedges), &fmt,
	  &ncon);
  ...
  *wgtflag = 0;
  ...
  vwgt = graph->vwgt =
    (readvw ? idxmalloc (ncon * graph->nvtxs, "ReadGraph: vwgt") : NULL);
  adjwgt = graph->adjwgt =
    (readew ? idxmalloc (graph->nedges, "ReadGraph: adjwgt") : NULL);
...
void
readMemberships (char *file, int numMemberships, idxtype
		 * members, idxtype ** groups_xadj, idxtype ** groupIds,
		 idxtype * ngroups)
{
  ...
  *groups_xadj = idxmalloc (*ngroups + 1, "readMemberships:groups_xadj");
...
/* Venu: my addition*/
void
my_WritePartition (const char *fname, idxtype * part, int n, float gamma)
{
  FILE *fpout;
  int i;
...
void
my_WritePartitionAddOne (const char *fname, idxtype * part, int n)
{
  FILE *fpout;
  int i;
...
void
my_WriteMappedPartition (const char *fname, idxtype * part, idxtype
			 * nodeMap, int n)
{
  FILE *fpout;
  int i;
 ...
 */
void
WriteRMap (const char *fname, idxtype * map, int n)
{
  int i, j;
...
**************************************************************************/
void
WritePartition (const char *fname, idxtype * part, int n, int nparts)
{
  FILE *fpout;
  int i;
  char filename[256];
...
**************************************************************************/
void
WriteMeshPartition (const char *fname, int nparts, int ne, idxtype * epart,
		    int nn, idxtype * npart)
{
  FILE *fpout;
  int i;
  char filename[256];
...
**************************************************************************/
void
WritePermutation (const char *fname, idxtype * iperm, int n)
{
  FILE *fpout;
  int i;
  char filename[256];
...
int
isGraphConnected (GraphType * graph)
{
...
idxtype *
getNodesToComponentMap (Matrix * graph1, int *numComponents, wgttype minWgt)
{
  sortAdjLists (graph1->nvtxs, graph1->xadj, graph1->adjncy, graph1->adjwgt);
  Matrix *graph2, *graph;
...
idxtype *
compSizeDistribution (GraphType * graph, int *numComponents)
{
  ...
  *numComponents = 0;
  ...
  free (visited);
  cSizes = (idxtype *) realloc (cSizes, (*numComponents) * sizeof (idxtype));
  return cSizes;
  ...
  for (i = 0; i < nvtxs; i++)
      ...
      for (j = xadj[i]; j < xadj[i + 1]; j++)
	{
	  ...
	  if (i == k)
	    {
	      printf
		("Vertex %d contains a self-loop (i.e., diagonal entry in the matrix)!\n",
		 i);
	      err++;
	    }
	  else
	      ...
	      for (l = xadj[k]; l < xadj[k + 1]; l++)
		{
		  if (adjncy[l] == i)
			...
			{
			  printf
			    ("Edges (%d %d) and (%d %d) do not have the same weight! %d %d\n",
...
//                abort();
  ...
  if (err > 0)
    printf
      ("A total of %d errors exist in the input file. Correct them, and run again!\n",
       err);
...
**************************************************************************/
idxtype *
ReadMesh (const char *filename, int *ne, int *nn, int *etype)
{
  int i, j, k, esize;
  idxtype *elmnts;
  FILE *fpin;
...
void
WriteMappedTxtGraphWithWts (const char *filename, int fullNvtxs,
			    idxtype * xadj, idxtype * adjncy,
			    idxtype * adjwgt, idxtype * map, int mappedNvtxs)
{
  int i, j;
  FILE *fpout;
...
void
WriteMatrix (const char *filename, int nvtxs, idxtype * xadj,
...
**************************************************************************/
void
WriteGraphWithWts (const char *filename, int nvtxs, idxtype * xadj,
...
**************************************************************************/
void
WriteTxtGraph (const char *filename, int nvtxs, idxtype * xadj,
	       idxtype * adjncy)
{
  int i, j;
  FILE *fpout;
...
**************************************************************************/
void
WriteGraph (const char *filename, int nvtxs, idxtype * xadj, idxtype * adjncy)
{
  int i, j;
  FILE *fpout;
...
void
printHistogram (idxtype * hist, int maxDegree, FILE * fp)
{
  int i;
...
void
writeFloats (float *a, int l, char *outputFile)
{
  FILE *fp;			// = stdout;
  ...
  for (int i = 0; i < l; i++)
    {
      // calculate 1-theta/pi. i.e. 1 - arccos(sim)/pi.
      if (a[i] > 1.0)		// sometimes, a little
	//rounding error here and there causes to be > 1.
	a[i] = 1.0;
//              float theta = 1.0 - acos(similarities[i])/PI;
//              fprintf(fp, "%.5f %.5f\n", similarities[i], theta);
      fprintf (fp, "%.5f\n", a[i]);
    }
  if (fp != stdout)
    fclose (fp);
