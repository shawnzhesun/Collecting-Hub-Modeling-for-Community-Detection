 ...
 * George
 * 
 * $Id: myqsort.c,v 1.2 2010-10-09 00:07:35 venu Exp $
 */
...
static void
siqst (idxtype * base, idxtype * max)
{
  register idxtype *i;
  register idxtype *j;
  register idxtype *jj;
...
**************************************************************************/
void
ikeysort (int n, KeyValueType * base)
{
...
static void
keyiqst (KeyValueType * base, KeyValueType * max)
{
  register KeyValueType *i;
  register KeyValueType *j;
...
**************************************************************************/
void
ikeyvalsort (int n, KeyValueType * base)
{
  register KeyValueType *i;
  register KeyValueType *j;
  register KeyValueType *lo;
...
static void
keyvaliqst (KeyValueType * base, KeyValueType * max)
{
  register KeyValueType *i;
  register KeyValueType *j;
  register KeyValueType *jj;
  ...
  do
      ...
      if (lo >= MTHRESH)
	{
	  j = (base->key > mid->key
	       || (base->key == mid->key
		   && base->val > mid->val) ? base : mid);
	  tmp = max - 1;
	  if (j->key > tmp->key || (j->key == tmp->key && j->val > tmp->val))
	    {
	      j = (j == base ? mid : base);	/* switch to first loser */
	      if (j->key < tmp->key
		  || (j->key == tmp->key && j->val < tmp->val))
		j = tmp;
	    }
      ...
      for (i = base, j = max - 1;;)
	{
	  while (i < mid
		 && (i->key < mid->key
		     || (i->key == mid->key && i->val <= mid->val)))
	    i++;
	  while (j > mid)
	    {
	      if (mid->key < j->key
		  || (mid->key == j->key && mid->val <= j->val))
		{
		  j--;
		  continue;
		}
