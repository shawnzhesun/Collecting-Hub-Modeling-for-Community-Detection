...
public:
  ...
  void buildMinSketches (const Matrix *, const CosHashVectors);
   ~CosineSketches ();
};
...
class BayesSimSearch
{
  ...
  float posteriorCdfUsingCaches (int nMatches, int nTrials, float x);
...
struct controldef
{
  ...
  /* Various Timers */
  timer TotalTmr, InitPartTmr, MatchTmr, ContractTmr, CoarsenTmr,
    UncoarsenTmr, SepTmr, RefTmr, ProjectTmr, SplitTmr, AuxTmr1, AuxTmr2,
    AuxTmr3, AuxTmr4, AuxTmr5, AuxTmr6;
...
typedef struct pairdef IntDoublePair;

// Type of which the bits of hash signature will be packed into
typedef unsigned long long int chunktype;
// Type of which the random projection elements will be stored at 
typedef unsigned char projtype;
