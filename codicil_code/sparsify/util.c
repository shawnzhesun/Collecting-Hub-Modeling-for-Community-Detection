...
void
mapIndices (idxtype * indices, idxtype * map, int n, int npart)
{
  int i = n - 1;
...
Matrix *
allocMatrix (int nvtxs, int nedges, int allocSum, int
...
int
checkSorted (int nvtxs, idxtype * xadj, idxtype * adjncy)
{
...
void
sortAdjLists (int nvtxs, idxtype * xadj, idxtype * adjncy, wgttype * adjwgt)
{
  int i, j;
  for (i = 0; i < nvtxs; i++)
    {
...
idxtype *
lookForSingletons (GraphType * graph, int *noOfSingletons)
{
...
void
dfTraversalMatrix (Matrix * graph, idxtype root, idxtype *
		   visited, int *nVisited, wgttype minWgt)
{
  ...
  for (i = graph->xadj[root]; i < graph->xadj[root + 1]; i++)
    {
      if (graph->adjwgt[i] >= minWgt && visited[graph->adjncy[i]] < 1)
	{
...
void
dfTraversal (GraphType * graph, idxtype root, idxtype * visited,
	     int *nVisited)
{
  int i;
  if (visited[root] < 1)
  ...
  for (i = graph->xadj[root]; i < graph->xadj[root + 1]; i++)
      ...
      if (visited[graph->adjncy[i]] < 1)
	{
	  visited[graph->adjncy[i]] = 1;
	  (*nVisited)++;
	  dfTraversal (graph, graph->adjncy[i], visited, nVisited);
	}
    }
}
...
**************************************************************************/
idxtype *
idxmalloc (int n, const char *msg)
{
  if (n == 0)
    return NULL;
...
**************************************************************************/
int *
ismalloc (int n, int ival, const char *msg)
{
  if (n == 0)
    return NULL;
...
**************************************************************************/
idxtype *
idxsmalloc (int n, idxtype ival, const char *msg)
{
  if (n == 0)
    return NULL;
...
**************************************************************************/
void *
GKmalloc (long nbytes, const char *msg)
{
  void *ptr;
  ...
  if (ptr == NULL)
    {
      ...
      errexit
	("***Memory allocation failed for %s. Requested size: %ld bytes", msg,
	 nbytes);
    }
...
**************************************************************************/
void *
GKmalloc (int nbytes, const char *msg)
{
  void *ptr;
...
**************************************************************************/
idxtype *
idxset (int n, idxtype val, idxtype * x)
{
  int i;
...
wgttype *
wgtset (int n, wgttype val, wgttype * x)
{
  int i;
...
**************************************************************************/
int
idxamax_strd (int n, idxtype * x, int incx)
{
  int i, max = 0;
...
**************************************************************************/
void
idxadd (int n, idxtype * x, idxtype * y)
{
  for (n--; n >= 0; n--)
    y[n] += x[n];
}
...
**************************************************************************/
float
ssum_strd (int n, float *x, int incx)
{
  int i;
  float sum = 0.0;
...
**************************************************************************/
void
sscale (int n, float alpha, float *x)
{
  int i;
...
void
permuteDegreeOrder (int n, idxtype * p, idxtype * xadj)
{
...
void
RandomPermuteFloatsInts (int n, wgttype * f, idxtype * a)
{
  int i, u, v;
  idxtype tmp;
  wgttype tmp_wgt;
...
**************************************************************************/
void
RandomPermute (int n, idxtype * p, int flag)
{
  int i, u, v;
  idxtype tmp;
...
void
RandomPermuteWgttype (int n, wgttype * p, int flag)
{
  int i, u, v;
  wgttype tmp;
...
int
bsearch_insertPos (idxtype * a, idxtype key, int start, int end)
{
  if (start > end)
    return start;
...
int
RandomPartitionInts (idxtype * a, int start, int end)
{
...
int
RandomPartition (wgttype * a, int start, int end)
{
  int n = end - start + 1, i, j;
  wgttype tmp, x;
  if (n < 2)
...
int
ParallelRandomPartitionInts (idxtype * a, idxtype * b, int start, int end)
{
  int n = end - start + 1, i, j;
  idxtype tmpidx, x;
  idxtype tmpwgt;
...
void
ParallelQSortInts (idxtype * a, idxtype * b, int start, int end)
{
  int q;
  if (start < end)
    {
...
int
ParallelRandomPartitionIntsUsingScores (idxtype * a, idxtype *
					b, idxtype * scores, int start,
					int end)
{
  int n = end - start + 1, i, j;
  idxtype tmpidx, x;
...
void
ParallelQSortIntsUsingScores (idxtype * a, idxtype * b, idxtype
...
int
RandomPartitionIntsUsingInts (idxtype * a, const idxtype * scores,
			      int start, int end)
{
  int n = end - start + 1, i, j;
  idxtype tmpidx, x;
...
void
QSortIntsUsingInts (idxtype * a, idxtype * scores, int start, int end)
{
  int q;
  if (start < end)
    {
...
int
ParallelRandomPartitionFloatsInts (wgttype * a, idxtype * b, int start,
				   int end)
{
  int n = end - start + 1, i, j;
  int tmpidx;
...
void
ParallelQSortFloatsInts (wgttype * a, idxtype * b, int start, int end)
{
  static int count = 0;
  int q;
  count++;
 ...
 * the order imposed by a. */
int
ParallelRandomPartitionLongs (long *a, wgttype * b, int start, int end)
{
  int n = end - start + 1, i, j;
  long tmpidx, x;
  wgttype tmpwgt;
...
void
ParallelQSortLongs (long *a, wgttype * b, int start, int end)
{
  int q;
  if (start < end)
    {
 ...
 * the order imposed by a. */
int
ParallelRandomPartition (idxtype * a, wgttype * b, int start, int end)
{
  int n = end - start + 1, i, j;
  idxtype tmpidx, x;
  wgttype tmpwgt;
...
/* Sorts the arrays a and b, according to the order imposed by a.
   ...
   the length of the two arrays is end-start+1, not end-start. */
void
ParallelQSort (idxtype * a, wgttype * b, int start, int end)
{
  int q;
  if (start < end)
    {
...
idxtype
RandomSelectInts (idxtype * a, int start, int end, int i)
{
  int q, k, j;
  if (start == end)
    return a[start];
...
wgttype
RandomSelect (wgttype * a, int start, int end, int i)
{
  int q, k, j;
  if (start == end)
    return a[start];
